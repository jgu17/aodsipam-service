...
# Helm chart config
HELM_REGISTRY ?= nccachepublicacr.azurecr.io
HELM_PREFIX ?= helm
...
install:
	helm install -n $(HELM_RELEASE_NAMESPACE) $(HELM_CHART_NAME) $(HELM_CHART_DIR) \
			--wait --timeout 15m; \

uninstall:
	helm uninstall -n $(HELM_RELEASE_NAMESPACE) $(HELM_CHART_NAME)
